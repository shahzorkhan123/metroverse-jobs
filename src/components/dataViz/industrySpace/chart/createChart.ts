...
const zoomScales = {
  continent: {
    fill: d3.scaleLinear().domain([1, 3]).range([1, 0]),
    stroke: d3.scaleLinear().domain([1, maxZoom]).range([1, 1]),
    label: d3.scaleLinear().domain([1, 1.85, 2.15]).range([1, 1, 0]),
  },
  countries: {
    fill: d3
      .scaleLinear()
    ...
    label: d3.scaleLinear().domain([1.45, 1.75, 3.65, 4]).range([0, 1, 1, 0]),
  },
  nodes: {
    label: d3.scaleLinear().domain([8, 9]).range([0, 1]),
  },
};
...
const createChart = (input: Input) => {
  ...
  const zoom = d3.zoom().scaleExtent([minZoom, maxZoom]).on("zoom", zoomed);
  ...
  const outerRing = g.append("circle").attr("class", "outer-ring");

  const innerRing = g.append("circle").attr("class", "inner-ring");

  //Create an SVG path (based on bl.ocks.org/mbostock/2565344)
  const outerRingLabelPath = g
    .append("path")
  ...
  const countries = g
    ...
    .on("click", (d) => {
      if (state.zoom > 1.5) {
        ...
        const continent = data.clusters.continents.find(
          (c) => c.clusterId === d.continent,
        );
    ...
    .on("mousemove", (d) => {
      ...
      if (d.numEmploy !== undefined) {
        rows.push([
          "Number of Employees:",
          formatNumber(Math.round(d.numEmploy)),
        ]);
      }
      ...
      tooltipEl.innerHTML = getStandardTooltip({
        ...
        boldColumns: [1],
        simple: true,
        additionalHTML: `<div style="padding: 0.35rem; font-style: italic; font-size: 0.7rem; text-align: center;">
  ...
  const nodes = g
    ...
    .on("mousemove", (d) => {
      ...
      const country = data.clusters.countries.find(
      ...
      const rows = [
        ...
        [
          "Low Aggregation<br />Knowledge Cluster:",
      ...
      if (state.colorBy === ColorBy.wage) {
        rows.push([
          "Hourly wage (based on U.S. average):",
          "$" + d.hourlyWage.toFixed(2),
        ]);
      }
      if (state.colorBy === ColorBy.education) {
        rows.push([
          "Years of education (based on U.S. average):",
          d.yearsEducation.toFixed(2),
        ]);
      }
      const clickText =
        state.active && state.active.datum && state.active.datum.id === d.id
          ? "Click to zoom out"
  ...
  const continentLabels = g
    .selectAll(".industry-continents-label")
    .data(data.clusters.continents)
    .enter()
    .append("text")
  ...
  const continentClusterLineLabels = g
    .selectAll(".industry-continents-cluster-label")
    .data(data.clusters.continents)
    .enter()
    .append("text")
    ...
    .attr(
      "y",
      (d) =>
        (yScale(d.center[1]) as number) + margin.top + textAndSpacingSize * 9.5,
    )
    .style("font-size", textAndSpacingSize * 9.5 + "px")
    .text("Cluster");

  const continentValueLabels = g
    .selectAll(".industry-continents-value-label")
    .data(data.clusters.continents)
    .enter()
    .append("text")
    ...
    .attr(
      "y",
      (d) =>
        (yScale(d.center[1]) as number) + margin.top + textAndSpacingSize * 20,
    )
  ...
  countryLabels
    .selectAll(".industry-countries-label")
    .data(data.clusters.countries)
    .enter()
    .append("text")
  ...
  const nodeLabels = g.append("g").attr("class", "industry-nodes-label-group");
  ...
  function zoomToPoint(d: any, external?: boolean) {
    ...
    const centerX = d.adjustedCoords
      ? d.adjustedCoords.x
      : (xScale(d.x) as number) + margin.left;
    const centerY = d.adjustedCoords
      ? d.adjustedCoords.y
      : (yScale(d.y) as number) + margin.top;
  ...
  const setHighlightedPoint = (id: string | undefined, action: NodeAction) => {
    if (id === undefined) {
      ...
      if (
        target !== -1 &&
        (state.active === null || state.active.datum.id !== id)
      ) {
  ...
  const setExternalHoveredId = (id: string | undefined) => {
    ...
    nodes
      ...
      .attr("stroke-width", (d) =>
        d.id === state.externalHoveredId ? 0.5 : null,
      );
  };
  ...
  function render(skipRingTransition?: boolean) {
    if (state.active) {
      const edgeData = state.active.datum.edges.map(
        ({ trg }: { trg: string }) => data.nodes.find(({ id }) => id === trg),
      );
      const centerX = state.active.datum.adjustedCoords
        ? state.active.datum.adjustedCoords.x
        : (xScale(state.active.datum.x) as number) + margin.left;
      const centerY = state.active.datum.adjustedCoords
        ? state.active.datum.adjustedCoords.y
        : (yScale(state.active.datum.y) as number) + margin.top;
      ...
      outerRingLabelPath.attr(
        "d",
        circlePath(centerX, centerY, outerRingRadius + 4),
      );

      innerRingLabelPath.attr(
        "d",
        circlePath(centerX, centerY, innerRingRadius + 4),
      );
      ...
      nodes
        ...
        .style("opacity", 1)
        .attr("r", (d) =>
          d.id === state.active.datum.id ||
          edgeData.find((e: { id: string }) => e.id === d.id)
            ? smallerRadius(d.radius ? d.radius * 0.5 : radius * 0.5)
            : radius,
        )
        .style("display", (d) =>
          d.id === state.active.datum.id ||
        ...
        .duration(500)
        .attr("cx", (d) =>
          (d as any).adjustedCoords
            ? (d as any).adjustedCoords.x
            : (xScale((d as any).x) as number) + margin.left,
        )
        .attr("cy", (d) =>
          (d as any).adjustedCoords
            ? (d as any).adjustedCoords.y
            : (yScale((d as any).y) as number) + margin.top,
        );
      ...
      countries.style("pointer-events", "none").style("opacity", 0);

      hoveredShape.style("display", "none");

      hoveredNode.style("display", "none");

      continentLabels.style("display", "none");
      continentClusterLineLabels.style("display", "none");
      continentValueLabels.style("display", "none");

      countryLabels.style("display", "none");

      nodeLabels.style("display", "none");

      state.active.element.style("display", "block");
      ...
      countries
        ...
        .attr(
          "stroke",
          rgba("#efefef", zoomScales.countries.stroke(state.zoom) as number),
        )
        .style("opacity", 1);
      ...
      if (state.zoom > 3.5) {
        nodeLabels.style("opacity", 1).style("display", "block");
      } else {
        nodeLabels.style("display", "none");
      }
      ...
      outerRingLabelPath.attr("d", "");
      innerRingLabelPath.attr("d", "");

      if (state.hoveredShape) {
        hoveredShape
          .attr(
            "points",
            state.hoveredShape.polygon
              .map(([xCoord, yCoord]: [number, number]) =>
                [
                  (xScale(xCoord) as number) + margin.left,
                  (yScale(yCoord) as number) + margin.top,
      ...
      if (state.hoveredNode) {
        hoveredNode
          ...
          .attr(
            "r",
            state.hoveredNode.radius ? state.hoveredNode.radius : radius,
          )
  ...
  function update(newData: SuccessResponse, rcaThreshold: number) {
    ...
    const totalEmployees = continentsData.reduce(
      (total, c) => (c.numEmploy ? c.numEmploy + total : total),
      0,
    );
    ...
    const intensityColorScaleContinents = d3
      .scaleSymlog()
      .domain(
        d3.extent(c1Rca.map((c) => (c.rca ? c.rca : 0))) as [number, number],
      )
      .range(intensityColorRange as any);

    const intensityColorScaleCountries = d3
      .scaleSymlog()
      .domain(
        d3.extent(c3Rca.map((c) => (c.rca ? c.rca : 0))) as [number, number],
      )
      .range(intensityColorRange as any);

    continents.each((d) => {
      const newDatum = c1Rca.find(
        ({ clusterId }) =>
      ...
      const countDatum = clusterData.find(
        ({ clusterId }) =>
          clusterId !== null && d.clusterId.toString() === clusterId.toString(),
      );
      if (countDatum && countDatum.numEmploy !== null) {
        d.numEmploy = countDatum.numEmploy;
        d.shareEmploy = parseFloat(
          ((countDatum.numEmploy / totalEmployees) * 100).toFixed(1),
        );
      }
    });

    continentValueLabels.text((d) => {
      const newDatum = clusterData.find(
        ({ clusterId }) =>
          clusterId !== null && d.clusterId.toString() === clusterId.toString(),
      );
      if (newDatum && newDatum.numEmploy !== null) {
        return (
          parseFloat(((newDatum.numEmploy / totalEmployees) * 100).toFixed(1)) +
          "% of employees"
        );
      } else {
    ...
    countries.each((d) => {
      const newDatum = c3Rca.find(
        ({ clusterId }) =>
          clusterId !== null && d.clusterId.toString() === clusterId.toString(),
      );
      if (newDatum && newDatum.rca !== null) {
        d.color = intensityColorScaleCountries(
          newDatum.rca,
        ) as unknown as string;
      } else {
        d.color = intensityColorRange[0];
    ...
    nodes
      .each((d) => {
        const newDatum = naicsRca.find(
          ({ naicsId }) =>
            naicsId !== null && d.id.toString() === naicsId.toString(),
        );
        if (newDatum && newDatum.rca !== null) {
          d.color =
            newDatum.rca >= rcaThreshold
              ? d.industryColor
              : lowIntensityNodeColor;
          d.rca = newDatum.rca;
        } else {
  ...
  function updateNodeSize(nodeSizing: NodeSizing, newData?: SuccessResponse) {
    ...
    if (newData && newData.naicsData) {
      if (nodeSizing === NodeSizing.rca) {
        const allValues = newData.naicsRca.map((c) =>
          c.rca ? c.rca : 0,
        ) as number[];
        radiusScale = d3
          .scaleSymlog()
        ...
        const allValues = newData.naicsData.map((c) =>
          c[field] ? c[field] : 0,
        ) as number[];
        radiusScale = d3
          .scaleLinear()
    ...
    nodes.each((d) => {
      if (nodeSizing === NodeSizing.globalCompanies) {
        d.radius =
          data.global.companySizeByScale(d.globalSumNumCompany) *
          radiusAdjuster;
      } else if (nodeSizing === NodeSizing.globalEmployees) {
        d.radius =
          data.global.employSizeByScale(d.globalSumNumEmploy) * radiusAdjuster;
      } else if (
        radiusScale &&
        newData &&
        newData.naicsRca &&
        ...
        const targetNode = newData.naicsRca.find(
          (dd) => dd.naicsId !== null && dd.naicsId.toString() === d.id,
        );
        d.radius =
          targetNode && targetNode.rca
            ? radiusScale(targetNode.rca as number)
            : radiusScale(0);
        ...
        d.radius =
          targetNode && targetNode[field]
            ? radiusScale(targetNode[field] as number)
            : radiusScale(0);
      } else {
        d.radius = radius;
      }
